...
import { ShoppingCart, Scan, Crosshair } from "lucide-react";
				...
				<Card
					...
					<div className="absolute top-0 left-1/2 -translate-x-1/2 w-1/2 h-[2px] flex justify-between opacity-50">
						{[...Array(5)].map((_, i) => (
							<div
								key={i}
								className="w-[2px] h-full bg-primary/50 group-hover:bg-primary transition-colors"
							/>
						))}
					</div>
					...
					<div className="relative aspect-square overflow-hidden bg-secondary/20">
						...
						<div className="absolute inset-x-0 bottom-0 z-20 translate-y-full p-4 transition-transform duration-300 group-hover:translate-y-0">
							<Button className="w-full gap-2 shadow-lg" size="sm">
								...
								<span className="font-mono text-xs">INIT_PURCHASE</span>
							</Button>
						</div>
					</div>
